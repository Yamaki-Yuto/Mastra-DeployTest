...
const MAX_RETRIES = 3; // Exponential backoff retries
...
function normalizeAndTruncatePrompt(input: string | undefined, maxLen = 1000): string {
  const normalized = String(input ?? '').replace(/\s+/g, ' ').trim();
  if (normalized.length <= maxLen) return normalized;
  const suffix = ' …';
  return normalized.slice(0, Math.max(0, maxLen - suffix.length)) + suffix;
}
...
export const geminiImageGenerationTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    // Normalize and trim prompt to avoid context/length issues
    let usedPrompt = normalizeAndTruncatePrompt(prompt, 800);

    console.log('[GeminiImageTool] Received input:');
    console.log(`[GeminiImageTool] Using prompt: "${usedPrompt.substring(0, 200)}${usedPrompt.length > 200 ? '...' : ''}"`);
    ...
    try {
      ...
      if (typeof seed === 'number') {
        console.warn('[GeminiImageTool] Seed is not supported by the API. The provided seed will be ignored.');
      }
      ...
      async function callOnce(body: unknown) {
        return axios.post(imagenApiUrl, body, {
      ...
      let primaryResponse;
      let attempt = 0;
      // Retry with exponential backoff on 429/5xx; if 400 length-related, truncate further once
      while (true) {
        try {
          // eslint-disable-next-line no-await-in-loop
          primaryResponse = await callOnce(requestBody);
          break;
        } catch (err: any) {
          const status = err?.response?.status;
          const text = err?.response?.data ? JSON.stringify(err.response.data) : String(err?.message || '');
          // If length/context related 400, try once with shorter prompt
          if (status === 400 && /context|length|too\s*long|max/i.test(text)) {
            const shorter = normalizeAndTruncatePrompt(usedPrompt, 400);
            if (shorter !== usedPrompt) {
              usedPrompt = shorter;
              requestBody.instances[0].prompt = usedPrompt;
              console.warn('[GeminiImageTool] 400 length issue. Retrying with shorter prompt.');
              // eslint-disable-next-line no-await-in-loop
              primaryResponse = await callOnce(requestBody);
              break;
            }
          ...
          if ((status === 429 || (status >= 500 && status < 600)) && attempt < MAX_RETRIES) {
            ...
            const delay = retryAfterMs || Math.floor(BASE_BACKOFF_MS * Math.pow(2, attempt) * (1 + Math.random() * 0.2));
            console.warn(`[GeminiImageTool] Retry ${attempt + 1}/${MAX_RETRIES} after ${delay}ms (status ${status}).`);
            // eslint-disable-next-line no-await-in-loop
            await sleep(delay);
            attempt += 1;
            continue;
      ...
      if (images.length > 0) {
        ...
        const successMessage = `Using prompt: ${usedPrompt}\n\n${images.length}枚の画像を生成しました！\n\n${markdownImages}`;
        ...
        return { 
          ...
          title: `${usedPrompt.substring(0, 30)}${usedPrompt.length > 30 ? '...' : ''}`,
          toolName: 'gemini-image-generation',
          toolDisplayName: 'Gemini画像生成',
          markdownImages
        };
      ...
      return {
        images: [],
        prompt: normalizeAndTruncatePrompt(prompt, 400),
        success: false,
        message: `Using prompt: ${normalizeAndTruncatePrompt(prompt, 400)}\n\nFailed to generate images: ${errorMessage}`,
        autoOpenPreview: false,
        error: errorMessage,
